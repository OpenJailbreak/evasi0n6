...
__attribute__((unused)) static void print_progress(uint64_t current, uint64_t total)
{
...
static int ignored_codes[16];

static int is_error_code_ignored(int ec)
{
	int i;
	if (ec == 0)
		return 0;
	for (i = 0; i < ignore_count; i++) {
		if (ignored_codes[i] == ec) {
			return 1;
		}
	}
	return 0;
...
// errorcode = 0 means delete all ignores
void idevicebackup2_set_ignore_error(int errorcode)
{
	if (errorcode == 0) {
		ignore_count = 0;
	} else {
		if (ignore_count == 16) {
			fprintf(stderr, "Sorry, can't ignore more than 16 error codes.\n");
			return;
		}
		ignored_codes[ignore_count++] = errorcode;
	}
}
...
checkpoint:
		...
		switch(cmd) {
			...
			if (cmd_flags & CMD_FLAG_RESTORE_REMOVE_ITEMS)
				...
				plist_dict_insert_item(opts, "RemoveItemsNotRestored", plist_new_bool(0));
		...
		if (cmd != CMD_LEAVE) {
			...
			do {
				...
				if (!strcmp(dlmsg, "DLMessageDownloadFiles")) {
					...
					for (ii = 0; ii < cnt; ii++) {
						...
						if (plist_get_node_type(val) == PLIST_STRING) {
							...
							if (str) {
								...
								int res = 0;
								if ((stat(newpath, &st) == 0) && S_ISDIR(st.st_mode))
									res = RemoveDirectoryA(newpath);
								else
									res = DeleteFileA(newpath);
								if (!res) {
					...
					if ((error_code != 0) && (!is_error_code_ignored(error_code))) {
						if (str) {
...
files_out:
	...
	if (is_error_code_ignored(-result_code)) {
		result_code = 0;
	}
