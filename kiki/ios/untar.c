...
void untar_set_lastmodified(const char* path, time_t lastmodified)
{
    ...
    TIMESPEC_TO_TIMEVAL(&times[0], &stat_buf.st_atimespec);
...
void untar_create_link_(char * fullpath, char * linkpath, time_t lastmodified, int filesize, FILE* fp){
...
void untar_create_directory_(char * fullpath, mode_t mode, time_t lastmodified, int filesize, FILE* fp){
    ...
    untar_set_lastmodified(fullpath, lastmodified);
}

void untar_create_file_(char * fullpath, mode_t mode, time_t lastmodified, int filesize, FILE*fp){
...
void untar(const char *filename, const char *basepath) {
    ...
    if(fp) {
        for(;;) {
            ...
            time_t lastmodified = strtoull(header.lastmodified, 0, 8);
            ...
            switch(header.linkindicator) {
                case '0':
                    untar_create_file_(fullpath, mode, lastmodified, filesize, fp);
                    break;
                case '2':
                    untar_create_link_(fullpath, header.linkedfile, lastmodified, filesize, fp);
                    break;
                case '5':
                    untar_create_directory_(fullpath, mode, lastmodified, filesize, fp);
                    break;
                default:
